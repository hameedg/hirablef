...
import CompetitionChallengesForm from './CompetitionChallengesForm';

const CompetitionChallengesEditForm = ({
  displayModal,
  setDisplayModal,
  data,
  badges,
  ...
  const handleSave = (e) => {
    ...
    if (currentFormData.result.trim() === '')
    ...
    setBadges({
      ...badges,
      competitionChallenges: newBadges,
    });
    setFormDataErrors({});
  ...
  const formatResult = (result) => {
    ...
    if (result === 'runnerUp') return 'Runner Up';
      ...
      >
        {currentFormData ? (
          ...
          <div className="m-[60px] flex flex-col">
            <p className="mb-[30px] font-medium">Competition/Challenges</p>
            {data.map((item, index) => (
              <div key={index} className="text-sm flex cursor-pointer mb-5">
                <p className="w-60">{item.name}</p>
                <p className="w-60">{formatResult(item.result)}</p>
...
export default CompetitionChallengesEditForm;
